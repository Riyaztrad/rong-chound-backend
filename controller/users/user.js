const express = require("express");
const fast2sms = require('fast-two-sms');
const User = require("../../models/user");
const { error } = require("console");



const getUsers = async (req, res, next) => {
  try {
    let users = await User.find({});

    if (users.length > 0) {
      return res.status(200).json({
        message: "users fetched successfully",
        data: users,
      });
    }

    return res.status(404).json({
      code: "BAD_REQUEST_ERROR",
      description: "No user found in the system",
    });
  } catch (error) {
    return res.status(500).json({
      code: "SERVER_ERROR",
      description: "something went wrong, Please try again",
    });
  }
};

const getUserById = async (req, res, next) => {
  try {
    let user = await User.findById(req.params.id);
    if (user) {
      return res.status(200).json({
        message: `user with id ${req.params.id} fetched successfully`,
        data: user,
      });
    }

    return res.status(404).json({
      code: "BAD_REQUEST_ERROR",
      description: "No user found in the system",
    });
  } catch (error) {
    return res.status(500).json({
      code: "SERVER_ERROR",
      description: "something went wrong, Please try again",
    });
  }
};

const createUser = async (req, res, next) => {
  try {
    const { name, mobile } = req.body;

    if (name === undefined || name === "") {
      return res.status(422).json({
        code: "REQUIRED_FIELD_MISSING",
        description: "name is required",
        field: "name",
      });
    }

    if (mobile === undefined || mobile === "") {
      return res.status(422).json({
        code: "REQUIRED_FIELD_MISSING",
        description: "mobile is required",
        field: "mobile",
      });
    }

    let isMobileExists = await User.findOne({
      mobile: mobile,
    });

    if (isMobileExists) {
      var option={
        authorization:"CnA5wHsz4Ne69PfFBxmJ1tiDyuGIaEbhMkWgj7qYTUXRQSL82Of9hSy6LTpJ3gOCwx1487jkXioDPqmB",
        message:"This Is A Otp ",
        numbers:[mobile]
       }
  
       fast2sms.sendMessage(option).then((response)=>{
        console.log(response)
       }).catch((error)=>{
        console.log(error)
       })
      return res.status(200).json({
        code: "ENTITY_ALREAY_EXISTS",
        description: "mobile already exists",
        field: "mobile",
      });
    }
    const otp = Math.floor(100000 + Math.random() * 900000) 
   
    const temp = {
      name: name,
      mobile: mobile,
      otp: otp,
    };
// sms send logic
     var option={
      authorization:"CnA5wHsz4Ne69PfFBxmJ1tiDyuGIaEbhMkWgj7qYTUXRQSL82Of9hSy6LTpJ3gOCwx1487jkXioDPqmB",
      message:"This Is A Otp code Message your otp code is 567890 ",
      numbers:['7499575375']
     }

     fast2sms.sendMessage(option).then((response)=>{
      console.log(response)
     }).catch((error)=>{
      console.log(error)
     })
    let newUser = await User.create(temp);

    if (newUser) {
      return res.status(201).json({
        message: "user created successfully",
        data: newUser,
      });
    } else {
      throw new Error("something went worng");
    }
  } catch (error){
    console.log(error)
  }
 {
    return res.status(500).json({
      code: "SERVER_ERROR",
      description: "something went wrong, Please try again",
    });
  }
};

const updateUser = async (req, res, next) => {
  try {
    const userId = req.params.id;

    const { name, mobile } = req.body;

    if (name === undefined || name === "") {
      return res.status(422).json({
        code: "REQUIRED_FIELD_MISSING",
        description: "name is required",
        field: "name",
      });
    }

    if (mobile === undefined || mobile === "") {
      return res.status(422).json({
        code: "REQUIRED_FIELD_MISSING",
        description: "mobile is required",
        field: "mobile",
      });
    }

    let isUserExists = await User.findById(userId);

    if (!isUserExists) {
      return res.status(404).json({
        code: "BAD_REQUEST_ERROR",
        description: "No user found in the system",
      });
    }

    const temp = {
      name: name,
      mobile: mobile,
    };

    let updateUser = await User.findByIdAndUpdate(userId, temp, {
      new: true,
    });

    if (updateUser) {
      return res.status(200).json({
        message: "user updated successfully",
        data: updateUser,
      });
    } else {
      throw new Error("something went worng");
    }
  } catch (error) {
    return res.status(500).json({
      code: "SERVER_ERROR",
      description: "something went wrong, Please try again",
    });
  }
};

const deleteUser = async (req, res, next) => {
  try {
    let user = await User.findByIdAndRemove(req.params.id);
    if (user) {
      return res.status(204).json({
        message: `user with id ${req.params.id} deleted successfully`,
      });
    }

    return res.status(404).json({
      code: "BAD_REQUEST_ERROR",
      description: "No users found in the system",
    });
  } catch (error) {
    return res.status(500).json({
      code: "SERVER_ERROR",
      description: "something went wrong, Please try again",
    });
  }
};

const generateOTP = (length = 4) => {
  let otp = "";

  for (let i = 0; i < length; i++) {
    otp += Math.floor(Math.random() * 10);
  }

  return otp;
};

module.exports = {
  getUsers: getUsers,
  getUserById: getUserById,
  createUser: createUser,
  updateUser: updateUser,
  deleteUser: deleteUser,
  generateOTP: generateOTP,
};
